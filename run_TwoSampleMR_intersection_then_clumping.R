...
##producing character vector of names of files in the directory
#files<-list.files(path="/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics", 
#                  pattern="*-Build37.f.tsv.gz", full.names = FALSE)

mbfile_list <- fread('/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/MiBioGen_' %&% args$mbfile %&% '.txt',
                     header = FALSE) %>% pull(V1)


#header_lines = "id.exposure,id.outcome,outcome,exposure,method,nsnp,b,se,pval"
#fwrite(header_lines, "MR_results_bonferroni_filtered.csv", quote = FALSE)

output_file_path = "/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/results_microbiome_and_PUKBB_hypertension_401/"
#file.create(output_file_path %&% "MR_results_bonferroni_filtered.csv")
#file.create("MR_results_bonferroni_filtered.csv")
#output_file_path = "/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/results_other/"
#File.Name = "continuous-1180-both_sexes.tsv.bgz"
File.Name2 = "phecode-401-both_sexes.tsv.bgz"
#sumstats1 = "continuous-1180-both_sexes"
sumstats = "phecode-401-both_sexes"
#phenotype1 = "sleep Chronotype"
phenotype2 = "Hypertension"
...
for (File.Name in mbfile_list){
  ...
  my_command_1 = "python3 /home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/00_mibiogen_panukbb_4_twosampleMR_heart.py --micro /home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/" %&% File.Name %&%" --panukb /home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/" %&% File.Name2 %&% " --output /home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/" %&% args$mbfile %&% "_" %&% sumstats %&% "_intersection"
  
  #call python script with system() in order to have a smaller file for read_outcome_data()
  #otherwise, a Cstack usage error
  system(my_command_1)
  
  filename_string = args$mbfile %&% "_" %&% sumstats %&% "_intersection.txt.gz"
  ...
  #change column input
  #microbiome_exp_dat$exposure <- "microbiome"
  #change column names
 # microbiome_exp_dat <- dplyr::rename(microbiome_exp_dat, rsid = SNP)
 # microbiome_exp_dat <- dplyr::rename(microbiome_exp_dat, pval = pval.exposure)
  
  
  ##outcome from IEU gwas
  ...
  chd_out_dat$outcome <- phenotype2
  ...
  #sumstats = "categorical-20002-both_sexes-1075"
  
  ##saving files one at a time
  if (dim(dat_for)[1] != 0) {
  fwrite(dat_for, file = output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_MR_data.csv", quote = FALSE, na = "NA")
  }
  if (dim(res_for)[1] != 0) {
  fwrite(res_for, file = output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_MR_results.csv", quote = FALSE, na = "NA")
  }
  exist_check_het_for <- exists("het_for")
  if (exist_check_het_for == TRUE && dim(het_for)[1] != 0) {
  fwrite(het_for, file = output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_MR_heterogeneity.csv", quote = FALSE, na = "NA")
  }
  exist_check_plt_for <- exists("plt_for")
  if (exist_check_plt_for == TRUE && dim(plt_for)[1] != 0) {
  fwrite(plt_for, file = output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_MR_pleiotropy.csv", quote = FALSE, na = "NA")
  }
  exist_check_sin_for <- exists("sin_for")
  if (exist_check_sin_for == TRUE && dim(sin_for)[1] != 0) {
  fwrite(sin_for, file = output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_MR_singlesnp.csv", quote = FALSE, na = "NA")
  }
  ...
  png(file= output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_scatterplot.png",
 ...
 # for (row in 1:nrow(res_for)) {
 #   if (res_for[row, 9] <= 0.05) {
 #     tally_for = tally_for + 1
 #   }
 # }
  
  #if (tally_for == 4) {
   # firsttimeornot = firsttimeornot + 1
   # if (firsttimeornot >= 1) {
  #  fwrite(res_for, file = "/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/results_microbiome_and_PUKBB_Myocardial_Infarction/results_1e-05/MR_results.csv", append = TRUE, quote = FALSE, na = "NA")
    #p1 <- mr_scatter_plot(res_for, dat_for)
    #png(file= output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_scatterplot_for.png",
       # width=600, height=350)
    #p1[[1]]
   # print(p1)
    #dev.off()
   # else 
   #   fwrite(res_for, file = "MR_results_bonferroni_filtered.csv", append = TRUE, quote = FALSE, na = "NA")
   # }
  #}
  ...
  ##saving files one at a time
  if (dim(dat_rev)[1] != 0) {
  fwrite(dat_rev, file = output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_MR_data.csv", quote = FALSE, na = "NA")
  }
  if (dim(res_rev)[1] != 0) {
  fwrite(res_rev, file = output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_MR_results.csv", quote = FALSE, na = "NA")
  }
  exist_check_het_rev <- exists("het_rev")
  if (exist_check_het_rev == TRUE && dim(het_rev)[1] != 0) {
  fwrite(het_rev, file = output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_MR_heterogeneity.csv", quote = FALSE, na = "NA")
  }
  exist_check_plt_rev <- exists("plt_rev")
  if (exist_check_plt_rev == TRUE && dim(plt_rev)[1] != 0) {
  fwrite(plt_rev, file = output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_MR_pleiotropy.csv", quote = FALSE, na = "NA")
  }
  exist_check_sin_rev <- exists("sin_rev")
  if (exist_check_sin_rev == TRUE && dim(sin_rev)[1] != 0) {
  fwrite(sin_rev, file = output_file_path %&% from_dict %&% "_vs_" %&% sumstats %&% "_MR_singlesnp.csv", quote = FALSE, na = "NA")
  }
  ...
  png(file= output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_scatterplot.png",
      width=600, height=550)
  p2[[1]]
  dev.off()
 # tally_rev = 0
 # firsttimeornot_rev = 0
  
  ##filtering for bonferroni all five tests with pval <= 0.05
#  for (row in 1:nrow(res_rev)) {
#    if (res_rev[row, 9] <= 0.05) {
 #     tally_rev = tally_rev + 1
  #  }
#  }
  
#  if (tally_rev == 4) {
   # firsttimeornot_rev = firsttimeornot_rev + 1
#    #if (firsttimeornot_rev >= 1) {
   # fwrite(res_rev, file = "/home/camilla/TwoSampleMR/MiBioGen_Summary_Statistics/results_microbiome_and_PUKBB_Myocardial_Infarction/results_1e-05/MR_results_bonferroni_filtered.csv", append = TRUE, quote = FALSE, na = "NA")
    #p1 <- mr_scatter_plot(res_rev, dat_rev)
    #png(file= output_file_path %&% sumstats %&% "_vs_" %&% from_dict %&% "_scatterplot_rev.png",
    #    width=600, height=350)
    #p1[[1]]
    #print(p1)
    #dev.off()
    ...
    #}
#  }
  ##saving results of all tests
  #fwrite(dat, file = "MR_data_GMB_vs_ieu-a-9.csv", append = TRUE)
  #fwrite(res, file = "MR_results_GMB_vs_ieu-a-9.csv", append = TRUE)
  #fwrite(het, file = "MR_heterogeneity_GMB_vs_ieu-a-9.csv", append = TRUE)
  #fwrite(plt, file = "MR_pleiotropy_GMB_vs_ieu-a-9.csv", append = TRUE)
  #fwrite(sin, file = "MR_singlesnp_GMB_vs_ieu-a-9.csv", append = TRUE)
